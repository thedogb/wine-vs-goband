...
// 查询置换表
int WineAI::ProbeHash(int depth, int alpha, int beta) {
...
// 写入置换表
void WineAI::RecordHash(int depth, int val, int hashf) {
  Hashe *phashe = &hashTable[zobristKey & (hashSize - 1)];
...
Pos WineAI::TurnBest() {
  ...
  cout << "MESSAGE" << " 棋型"  << "0:" << cell[best.x+4][best.y+4].pattern[who][0] << endl;
  cout << "MESSAGE" << " 棋型"  << "1:" << cell[best.x+4][best.y+4].pattern[who][1] << endl;
  cout << "MESSAGE" << " 棋型"  << "2:" << cell[best.x+4][best.y+4].pattern[who][2] << endl;
  cout << "MESSAGE" << " 棋型"  << "3:" << cell[best.x+4][best.y+4].pattern[who][3] << endl;
...
// 根节点搜索
Point WineAI::minimax(int depth, int alpha, int beta, Line *pline) {
...
// 获取下一步着法
Pos WineAI::MoveNext(MoveList &moveList){
    /*phase0:置换表着法
     *phase1:生成全部着法
...
// 记录pv着法
void WineAI::RecordPVS(Pos best){
...
//带pvs的搜索
int WineAI::AlphaBeta(int depth, int alpha, int beta, Line *pline) {
  total++;
...
// 棋型剪枝
int WineAI::CutCand(Pos * move, Point * cand, int candCount) {
  int moveCount = 0;
...
// 插入排序
void WineAI::sort(Point * a, int n) {
  int i, j;
  Point key;
  for (i = 1; i < n; i++) {
    key = a[i];
